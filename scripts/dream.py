...
from torchvision.models import resnet50, ResNet50_Weights, vgg19, VGG19_Weights, densenet121, DenseNet121_Weights, inception_v3, Inception_V3_Weights
# from config import load_config
# from scripts.depth_inference import MiDaS
from scripts.spyNet import calc_opflow
from scripts.dream_utility import clip, convert, deprocess, get_octaves, preprocess, warp
...
class Dreamer:
    ...
    def init_model(self):
        ...
        relative_path = os.path.join("models","headtrip")

        if self.config["model"] == "resnet":

            # model_path = os.path.join(current_dir, relative_path, "resnet50.pt")
            # if not os.path.exists(model_path): 
            #     torch.save(resnet50(weights=ResNet50_Weights.IMAGENET1K_V1), model_path)

            # network = safe.unsafe_torch_load("models/headtrip/resnet50.pt")

            network = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)

        elif self.config["model"] == "vgg19":

            # model_path = os.path.join(current_dir, relative_path, "vgg19.pt")
            # if not os.path.exists(model_path): 
            #     torch.save(vgg19(weights=VGG19_Weights.IMAGENET1K_V1), model_path)

            network = safe.unsafe_torch_load("models/headtrip/vgg19.pt")

        elif self.config["model"] == "densenet":
            network = safe.unsafe_torch_load("models/headtrip/densenet.pt")

        # elif self.config["model"] == "inception":
        #     network = inception_v3(weights=Inception_V3_Weights.IMAGENET1K_V1)
    ...
    def dream_single(self):
        ...
        img = self.deep_dream(self.transform(self.img), self.model, 0, seq="first")
        img = deprocess(img)
        img = np.clip(img, 0, 1)
        return img
...
def start_dreamer(img, config):
    ...
    dreamer = Dreamer(img, config)
    
    if config["seq"]:
        dreamer.dream_seq()
    else:
        return dreamer.dream_single()
